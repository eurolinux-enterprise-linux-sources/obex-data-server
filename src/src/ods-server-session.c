...
{
	g_source_remove (server_session->priv->io_watch);
...
static void
setup_transport (OdsServerSession *server_session)
{
	OdsObexContext *obex_context;
	GIOChannel *chan;
	gint ret;
	...
	chan = g_io_channel_unix_new (server_session->priv->fd);
	g_io_channel_set_close_on_unref (chan, TRUE);
	server_session->priv->io_watch = g_io_add_watch(chan,
...
ods_server_session_cancel_internal (OdsServerSession *server_session)
{
	...
	server_session->priv->obex_context->cancelled = TRUE;
