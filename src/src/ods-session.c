...
#define ODS_SESSION_LOCK(session) g_message ("LOCK %s", __FUNCTION__); g_static_mutex_lock (&(session)->priv->mutex)
#define ODS_SESSION_UNLOCK(session) g_message ("UNLOCK %s", __FUNCTION__); g_static_mutex_unlock (&(session)->priv->mutex)
...
struct OdsSessionPrivate
{
	...
	gboolean				usbreadmore; /* Used to track reading from USB - io_watch is not possible */
	/* state variables */
					...
					int response)
{
	...
	g_message ("obex_request_done: command %d, response %d (%s)", command,
				response, OBEX_ResponseToString (response));
			...
			int command, int response)
{
	...
	switch (event) {
		...
		case OBEX_EV_PARSEERR:
			g_warning ("EV_PARSEERR");
			break;
		case OBEX_EV_ACCEPTHINT:
			g_warning ("EV_ACCEPTHINT");
			break;
			...
			int command, int response)
{
	...
	if (event == OBEX_EV_PROGRESS ||
		...
		event == OBEX_EV_STREAMAVAIL)
			session->priv->usbreadmore = TRUE; /* There is more data to read */
	else
			session->priv->usbreadmore = FALSE;
}

static void
ods_session_setup_fdtransport (OdsSession *session)
{
	OdsObexContext *obex_context;
	GIOChannel *chan;
	gint ret;
	...
	ods_session_connect_internal (session, &error);
...
static void
ods_session_setup_usbtransport (OdsSession *session)
{
	OdsObexContext *obex_context;
	gint ret;
	GError *error = NULL;
	obex_interface_t *obex_intf;
	int interfaces_num, intf_num;

	obex_context = session->priv->obex_context;
	intf_num = session->priv->usbintfnum;
	session->priv->io_watch = 0;

	/* call OBEX_Init, setup USB Transport here */
	obex_context->obex_handle = OBEX_Init (OBEX_TRANS_USB, obex_usbevent, 0);
	if (obex_context->obex_handle == NULL) {
		g_set_error (&error, ODS_ERROR, ODS_ERROR_FAILED, "Out of memory");
		goto err;
	}
                        ...
                        GParamSpec   *pspec)
{
	...
	switch (property_id) {
		case ODS_SESSION_FD:
			...
			if (self->priv->fd >= 0)
				ods_session_setup_fdtransport (self);
			break;
		...
		case ODS_SESSION_USBINTFNUM:
			...
			if (self->priv->usbintfnum >= 0)
				ods_session_setup_usbtransport (self);
			break;
		default:
 ...
 **/
gboolean ods_process_usb_input(gpointer *data)
{
	OdsSession *session;
	obex_t* obex_handle;

	obex_handle = (obex_t *) data;
	int i = OBEX_HandleInput (obex_handle, 1);
	if (i <= 0) {
		/* Link error: for example USB cable was disconnected */
		obex_io_callback(NULL, G_IO_ERR, data);
		return FALSE;
	}

	session = ODS_SESSION (OBEX_GetUserData (obex_handle));

	return session->priv->usbreadmore;
}
...
ods_session_cancel_internal (OdsSession *session)
{
	...
	/* Send CMD_ABORT at obex_writestream/readstream;
	* cleanup will be done in obex_event */
	session->priv->obex_context->cancelled = TRUE;
	return TRUE;
}
