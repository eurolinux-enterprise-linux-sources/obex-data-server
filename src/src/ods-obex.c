...
ods_obex_send(obex_t *obex_handle, obex_object_t *object)
{
	...
	if (err >=0 ) {
		...
		if (usbintfnum >= 0)
			...
			g_idle_add((GSourceFunc)ods_process_usb_input, obex_handle);
	}
...
void
ods_obex_transfer_suspend (OdsObexContext *obex_context)
{
	g_message ("suspending request");
	while (!obex_context->suspend_result)
		g_main_context_iteration (NULL, TRUE);
	g_message ("suspend result: %d", obex_context->suspend_result);
}
